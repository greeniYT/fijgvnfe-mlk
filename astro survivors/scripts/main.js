  ...
  window.RuntimeInterface = class RuntimeInterface {
    constructor(opts) {
      ...
      if (this._exportType === "cordova" && this._useWorker)
        ...
        } if (this.IsWebView2Wrapper()) self["chrome"]["webview"].addEventListener("message",
    ...
    async _Init(opts) {
      ...
      if (opts.runtimeBaseUrl) this._runtimeBaseUrl =
        opts.runtimeBaseUrl;
      else {
      ...
      if (window["c3_addPortMessageHandler"]) window["c3_addPortMessageHandler"](e =>
    ...
    _DoUpdateHTMLElementsZOrderOnHTMLLayer(htmlIndex, arr) {
      ...
      const htmlWrap =
        this.GetHTMLWrapElement(htmlIndex);
    ...
    PostToRuntimeComponent(component,
...
'use strict';
{
  ...
  const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
    constructor(iRuntime) {
      ...
      if (typeof PointerEvent !== "undefined") window.addEventListener("pointerdown", PreventDefaultOnCanvasOrDoc, {
        "passive": false
      });
    ...
    _AddDefaultCanvasEventHandlers(canvas) {
      ...
      if (typeof PointerEvent !== "undefined") canvas.addEventListener("pointerdown", PreventDefaultOnCanvasOrDoc);
    ...
    Attach() {
      ...
      if (typeof PointerEvent !== "undefined") {
        ...
        if (this._iRuntime.UsesWorker() && typeof window["onpointerrawupdate"] !== "undefined" && self === self.top) window.addEventListener("pointerrawupdate",
          e => this._OnPointerRawUpdate(e));
        ...
        window.addEventListener("mouseup",
          e => this._OnMouseEventAsPointer("pointerup", e));
    ...
    _OnKeyEvent(name, e) {
      ...
      if (this._isExportToVideo) return;
      const code =
        KEY_CODE_ALIASES.get(e.code) || e.code;
